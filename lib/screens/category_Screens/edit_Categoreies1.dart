...
class Edit_Category_Screen extends StatefulWidget {

final Catogory catogory;

Edit_Category_Screen({required this.catogory});

  @override
  _Edit_Category_ScreenState createState() => _Edit_Category_ScreenState();
}

class _Edit_Category_ScreenState extends State<Edit_Category_Screen> with Helpers{
...
@override
  void initState() {
    ...
    super.initState();
    category_Contraller = TextEditingController(text: widget.catogory.title);
    description_Contraller = TextEditingController(text: widget.catogory.subTitle);
...
@override
  ...
  Widget build(BuildContext context) {
    return  Scaffold(
      ...
      body: BlocListener<Catagory_bloc,Crud_State>(
        listenWhen: (previous, current) => current is ProcessState && current.processType == ProcessType.update,
        ...
        child: SingleChildScrollView(
          child: Padding(
            ...
            child: Column(
              children: [
                Text_bold(text: 'old Category'),
                SizedBox(height: SizeConfig.scaleHeight(15)),
                Text_Light(text: 'Edit category' ,textSize: 18,),
  ...
  void save() {
        // Catgories_GetXController.to.updateCatgory(catgorey);
        // Provider.of<Provider_Notifers_Catgories>(context,listen: false).update(catgorey);
        // Helpers.ShowSnackBar(context, 'Has been Updated',error: false);
    BlocProvider.of<Catagory_bloc>(context).add(Update_Events(catgorey));
  ...
  Catogory get catgorey{
    ...
    cat.id = widget.catogory.id;
    cat.user_id = widget.catogory.user_id;
    return cat;
  }}